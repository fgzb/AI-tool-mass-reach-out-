...
```
 collect   ─►  enrich (one pass)        ─►  send                ◄─ reads replies, opt-outs, bounces
 French         find & verify the website    first e-mail (A/B),
 registries     + crawl it for pro e-mails   one follow-up in the same thread,
 (API, SIRENE,                               daily quota over office hours,
 Pappers, CSV)                               warm-up, auto-pause, daily report
```
...
3. **Sends at most one e-mail** (a first e-mail or a follow-up), and only inside the sending window:
...
4. **E-mails you a daily report** after 17:00: sent vs. quota, replies (with company names), opt-outs, bounces, 7-day rates, how many contacts are ready and, with A/B testing, the reply rate of each template.
5. **Keeps contacts ready in advance.** When fewer than 150 contacts are ready, it:
   - finds the website and crawls it for e-mails, in one pass, for companies not processed yet;
   - otherwise pulls the next page of your registry search.
...
### Follow-ups and A/B testing

- **Follow-up:** 4 business days after the first e-mail, companies that did not reply, opt out or bounce get **one** short follow-up, in the same thread (`Re: …`), from `templates/relance_fr.txt`. The inbox is re-read just before each follow-up, so nobody who has just replied gets one. A follow-up that could not go out within 14 days is dropped. Follow-ups use at most half of each day's quota, so new companies keep being contacted.
  - The trade-off: over the first 2 months, the same ~3,600 messages reach about **1,900 companies + 1,700 follow-ups**, instead of 3,600 companies with no follow-up. In cold e-mail a single follow-up usually brings a large share of the replies. If you prefer maximum reach, set `mail.followup.enabled: false`.
- **A/B testing:** list two or more templates in `mail.templates`. Each company always gets the same variant, and the daily report shows sent / replies / reply rate / opt-outs per variant. After a few hundred e-mails per variant, keep the winner and try a new challenger.
...
| Volume after the first 2 months | about 3,600 messages, with the domain still healthy: ~1,900 companies + ~1,700 follow-ups (or 3,600 companies with follow-ups off) |
...
3. Run `fr-outreach agent` (rehearsal) for a day and read the `.eml` files in `outbox/<campaign>/`: each first e-mail comes with its `_relance.eml` follow-up.
4. Replace every `[PLACEHOLDER]` in the templates (first e-mail and follow-up). Live mode refuses to start while any remain.
...
1. **Website candidates:**
   ...
   Guessed domains are checked in DNS first (a few milliseconds), so only domains that exist are loaded; parked / "domain for sale" pages are rejected.
...
3. **E-mail crawl**, in the same pass: the pages loaded for verification are reused from a cache, contact pages come first, and the crawl stops as soon as a role address on the company's own domain is found (about 5 requests per company instead of 11). At most 6 pages per site. The crawler respects robots.txt, waits between requests to the same site and uses an honest User-Agent. It decodes `mailto:` links, Cloudflare-protected addresses and `nom [at] domaine [dot] fr` forms.
4. **Ranking:**
...
fr-outreach enrich                    # website + e-mails in one pass (or `discover` then `scrape`)
...
fr-outreach send --send               # real send (due follow-ups + first e-mails), limited by today's quota
...
- One address per company, at most one follow-up, no re-contact for 6 months, sole traders excluded, companies marked non-public in SIRENE skipped.
...
Performance, measured on 50,000 companies (SQLite in WAL mode, bulk writes batched):

| | |
|---|---|
| picking the next contact / counting ready contacts | 0.4 s (scales linearly: 1.6 s at 200,000) |
| importing 50,000 companies | 3 s |
| HTTP requests per company (website + e-mails) | about 5 |

SIRENE imports with a department or postal-code filter read the establishments file first, so only the head offices of that area are held in memory, not every matching company in France.
...
| `pipeline.py` | collect / enrich (discover + scrape) batches |
| `discovery.py` | website finding and verification |
| `scraper.py`, `emails.py` | crawling, extraction and ranking |
| `mailer.py` | templates, A/B variants, follow-ups, safeguards, SMTP, quota |
